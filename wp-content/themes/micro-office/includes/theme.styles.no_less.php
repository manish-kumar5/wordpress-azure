...
if (!function_exists('micro_office_action_theme_styles_no_less_theme_setup')) {
	add_action( 'micro_office_action_before_init_theme', 'micro_office_action_theme_styles_no_less_theme_setup', 1 );
	function micro_office_action_theme_styles_no_less_theme_setup() {

		// If no LESS support
		if (micro_office_get_theme_setting('less_compiler') == 'no') {
			// Add theme styles
			add_action('micro_office_action_add_styles', 				'micro_office_action_theme_styles_no_less_add_styles');

			// Add colors and fonts for the WP Customizer
			add_action( 'customize_controls_print_footer_scripts',	'micro_office_customizer_wp_css_template' );
			add_action( 'micro_office_filter_get_css',					'micro_office_customizer_wp_add_scheme_in_css', 100, 4 );
		}
	}
}
...
if (!function_exists('micro_office_action_theme_styles_no_less_add_styles')) {
	
	function micro_office_action_theme_styles_no_less_add_styles() {
		// Add stylesheet files
		if ( !is_customize_preview() && !isset($_GET['color_scheme']) && micro_office_param_is_off(micro_office_get_theme_option('debug_mode')) ) {
			...
			wp_add_inline_style( 'micro_office-main-style', micro_office_get_custom_css() . micro_office_get_inline_css() );
		}
	}
}
...
if (!function_exists('micro_office_customizer_wp_add_scheme_in_css')) {
	
	function micro_office_customizer_wp_add_scheme_in_css($css, $colors, $fonts, $scheme) {
		$rez = '';
		$in_comment = $in_rule = false;
		$allow = true;
		$scheme_class = '.scheme_' . trim($scheme) . ' ';
		$self_class = '.scheme_self';
		$self_class_len = micro_office_strlen($self_class);
		...
		for ($i=0; $i<strlen($css_str); $i++) {
			...
			if ($in_comment) {
				...
				if ($ch=='/' && $css_str[$i+1]=='*') {
					...
					$rez .= $ch;
					$allow = true;
				} else if (micro_office_strpos(" \t\r\n", $ch)===false) {
					if ($allow && micro_office_substr($css_str, $i, $self_class_len) == $self_class) {
...
if ( !function_exists( 'micro_office_customizer_wp_css_template' ) ) {
	
	function micro_office_customizer_wp_css_template() {
		...
		$schemes = array_keys(micro_office_get_list_color_schemes());
		if (count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<'.trim($tmpl_holder).' type="text/html" id="tmpl-micro_office-color-scheme-'.esc_attr($scheme).'">'
						. trim(micro_office_get_custom_css( $colors, false, false, $scheme ))
					. '</'.trim($tmpl_holder).'>';
			}
		}
		...
		if (is_array($custom_fonts) && count($custom_fonts) > 0) {
			...
			echo '<'.trim($tmpl_holder).' type="text/html" id="tmpl-micro_office-fonts">'
					. trim(micro_office_get_custom_css( false, $fonts, false, false ))
				. '</'.trim($tmpl_holder).'>';
		}
...
if (!function_exists('micro_office_customizer_wp_add_theme_colors')) {
	function micro_office_customizer_wp_add_theme_colors($colors) {
		if (micro_office_substr($colors['text'], 0, 2) == '{{') {
			...
			$colors['text_dark_0_05']	= micro_office_hex2rgba( $colors['text_dark'], 0.05 );
			$colors['text_dark_0_1']	= micro_office_hex2rgba( $colors['text_dark'], 0.1 );
			$colors['text_dark_0_8']	= micro_office_hex2rgba( $colors['text_dark'], 0.8 );
			$colors['text_link_0_3']	= micro_office_hex2rgba( $colors['text_link'], 0.3 );
			$colors['text_link_0_6']	= micro_office_hex2rgba( $colors['text_link'], 0.6 );
			$colors['text_link_0_8']	= micro_office_hex2rgba( $colors['text_link'], 0.8 );
			$colors['text_hover_0_2']	= micro_office_hex2rgba( $colors['text_hover'], 0.2 );
			$colors['text_hover_0_3']	= micro_office_hex2rgba( $colors['text_hover'], 0.3 );
			$colors['text_hover_0_8']	= micro_office_hex2rgba( $colors['text_hover'], 0.8 );
			$colors['inverse_text_0_1']	= micro_office_hex2rgba( $colors['inverse_text'], 0.1 );
			$colors['bg_color_0_8']		= micro_office_hex2rgba( $colors['bg_color'], 0.8 );
			$colors['alter_text_0_1']	= micro_office_hex2rgba( $colors['alter_text'], 0.1 );
			$colors['alter_bg_color_0_8'] = micro_office_hex2rgba( $colors['alter_bg_color'], 0.8 );
			$colors['alter_bg_hover_0_5'] = micro_office_hex2rgba( $colors['alter_bg_hover'], 0.5 );
			$colors['alter_bd_color_0_1'] = micro_office_hex2rgba( $colors['alter_bd_color'], 0.1 );
		}
		return $colors;
	}
}
...
if (!function_exists('micro_office_customizer_wp_add_theme_fonts')) {
	function micro_office_customizer_wp_add_theme_fonts($fonts) {
		if (micro_office_substr($fonts['h1_font-family'], 0, 2) == '{{') {
			...
			$fonts['logo_margin-top_0_75']	 = micro_office_multiply_css_value( $fonts['logo_margin-top_value'], 0.75 );
			$fonts['logo_margin-bottom_0_5'] = micro_office_multiply_css_value( $fonts['logo_margin-bottom_value'], 0.5 );
			$fonts['menu_margin-top_1']		 = $fonts['menu_margin-top_value'];
			$fonts['menu_margin-top_0_3']	 = micro_office_multiply_css_value( $fonts['menu_margin-top_value'], 0.3 );
			$fonts['menu_margin-top_0_6']	 = micro_office_multiply_css_value( $fonts['menu_margin-top_value'], 0.6 );
			$fonts['menu_margin-top_0_65']	 = micro_office_multiply_css_value( $fonts['menu_margin-top_value'], 0.65 );
			$fonts['menu_margin-top_0_7']	 = micro_office_multiply_css_value( $fonts['menu_margin-top_value'], 0.7 );
			$fonts['menu_margin-top_0_8']	 = micro_office_multiply_css_value( $fonts['menu_margin-top_value'], 0.8 );
			$fonts['menu_margin-bottom_1']	 = $fonts['menu_margin-bottom_value'];
			$fonts['menu_margin-bottom_0_5'] = micro_office_multiply_css_value( $fonts['menu_margin-bottom_value'], 0.5 );
			$fonts['menu_margin-bottom_0_6'] = micro_office_multiply_css_value( $fonts['menu_margin-bottom_value'], 0.6 );
			$fonts['menu_height_1']			 = micro_office_summ_css_value( micro_office_summ_css_value( $fonts['menu_margin-top_value'],  $fonts['menu_margin-bottom_value'] ), $fonts['menu_line-height_value'] );
...
if (!function_exists('micro_office_get_custom_css')) {

	function micro_office_get_custom_css($colors=null, $fonts=null, $minify=true, $only_scheme='') {

		$add_comment = $colors===null && $fonts===null && empty($only_scheme)
						? '/* ' . strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'micro-office') ) . "*/\n"
						: '';
		...
		// Prepare fonts
		if ($fonts===null) {
			$fonts_list = micro_office_get_list_fonts(false);
			$custom_fonts = micro_office_get_custom_fonts();
			$fonts = array();
			foreach ($custom_fonts as $tag => $font) {
				$fonts[$tag.'_font-family'] = !empty($font['font-family']) && !micro_office_is_inherit_option($font['font-family'])
												? "font-family:\"" . str_replace(' ('.esc_html__('uploaded font', 'micro-office').')', '', $font['font-family']) . '"' 
													. (isset($fonts_list[$font['font-family']]['family']) ? ',' . $fonts_list[$font['font-family']]['family'] : '' ) 
													. ";"
												: '';
				$fonts[$tag.'_font-size'] = !empty($font['font-size']) && !micro_office_is_inherit_option($font['font-size'])
												? "font-size:" . micro_office_prepare_css_value($font['font-size']) . ";"
												: '';
				$fonts[$tag.'_line-height'] = !empty($font['line-height']) && !micro_office_is_inherit_option($font['line-height'])
												? "line-height: " . micro_office_prepare_css_value($font['line-height']) . ";"
												: '';
				$fonts[$tag.'_line-height_value'] = !empty($font['line-height'])	// && !micro_office_is_inherit_option($font['line-height'])
												? micro_office_prepare_css_value($font['line-height'])
												: 'inherit';
				$fonts[$tag.'_font-weight'] = !empty($font['font-weight']) && !micro_office_is_inherit_option($font['font-weight'])
												? "font-weight: " . trim($font['font-weight']) . ";"
												: '';
				$fonts[$tag.'_font-style'] = !empty($font['font-style']) && !micro_office_is_inherit_option($font['font-style']) && micro_office_strpos($font['font-style'], 'i')!==false
												? "font-style: italic;"
												: '';
				$fonts[$tag.'_text-decoration'] = !empty($font['font-style']) && !micro_office_is_inherit_option($font['font-style']) && micro_office_strpos($font['font-style'], 'u')!==false
												? "text-decoration: underline;"
												: '';
				$fonts[$tag.'_margin-top'] = !empty($font['margin-top']) && !micro_office_is_inherit_option($font['margin-top'])
												? "margin-top: " . micro_office_prepare_css_value($font['margin-top']) . ";"
												: '';
				$fonts[$tag.'_margin-top_value'] = !empty($font['margin-top'])	
												? micro_office_prepare_css_value($font['margin-top'])
												: 'inherit';
				$fonts[$tag.'_margin-bottom'] = !empty($font['margin-bottom']) && !micro_office_is_inherit_option($font['margin-bottom'])
												? "margin-bottom: " . micro_office_prepare_css_value($font['margin-bottom']) . ";"
												: '';
				$fonts[$tag.'_margin-bottom_value'] = !empty($font['margin-bottom'])	
												? micro_office_prepare_css_value($font['margin-bottom'])
												: 'inherit';
			}
		}
		if ($fonts) {
			$fonts = micro_office_customizer_wp_add_theme_fonts($fonts);
...
FONTS;
		...
		if ($colors!==false) {
			$schemes = empty($only_scheme) ? array_keys(micro_office_get_list_color_schemes()) : array($only_scheme);
			if (count($schemes) > 0) {
				$step = 1;
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = micro_office_get_scheme_colors($scheme);

					// Make theme-specific colors and tints
					$colors = micro_office_customizer_wp_add_theme_colors($colors);
...
CSS;
					...
					$rez = apply_filters('micro_office_filter_get_css', $rez, $colors, $fonts, $scheme);
		...
		if (!empty($css_str))
			$css_str = $add_comment . ($minify ? micro_office_minify_css($css_str) : $css_str);
