<?php
// Get template args
extract(micro_office_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = micro_office_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			<?php 
			if ($page_style!='single-portfolio-fullscreen') micro_office_show_layout($post_data['post_thumb']);
			$cur = get_post();
			$args = array(
			...
			$args = micro_office_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = micro_office_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = micro_office_strshort($next->post_title, 30);
				?>
