...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_demo_type input[type="radio"]').on('change', function() {
		...
		var data = {
			ajax_nonce: MICRO_OFFICE_STORAGE['ajax_nonce'],
			action: 'micro_office_importer_get_list_pages',
			demo_type: type
		};
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		MICRO_OFFICE_STORAGE['importer_error_messages'] = '';
		MICRO_OFFICE_STORAGE['importer_ignore_errors'] = true;
		micro_office_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function micro_office_importer_do_action(steps, idx) {
	"use strict";
	...
	var data = {
		ajax_nonce: MICRO_OFFICE_STORAGE['ajax_nonce'],
		action: 'micro_office_importer_start_import',
	...
	jQuery.post(MICRO_OFFICE_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || MICRO_OFFICE_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				MICRO_OFFICE_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				micro_office_importer_do_action(steps, idx);
			} else {
				if (MICRO_OFFICE_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + MICRO_OFFICE_STORAGE['importer_error_msg'] + '</h4>' + MICRO_OFFICE_STORAGE['importer_error_messages']);
				} else {
