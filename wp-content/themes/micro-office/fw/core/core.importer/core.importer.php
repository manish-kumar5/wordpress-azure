...
// Theme init
if (!function_exists('micro_office_importer_theme_setup')) {
	add_action( 'micro_office_action_after_init_theme', 'micro_office_importer_theme_setup' );		// Fire this action after load theme options
	function micro_office_importer_theme_setup() {
		if (is_admin() && current_user_can('import') && micro_office_get_theme_option('admin_dummy_data')=='yes') {
			new micro_office_dummy_data_importer();
		}
	}
}

class micro_office_dummy_data_importer {
	...
	function __construct() {
		...
		// AJAX handler
		add_action('wp_ajax_micro_office_importer_start_import',		array($this, 'importer'));
		add_action('wp_ajax_nopriv_micro_office_importer_start_import',	array($this, 'importer'));
		// AJAX handler of the get_list_pages actions
		add_action('wp_ajax_micro_office_importer_get_list_pages',		array($this, 'get_list_pages_callback'));
		add_action('wp_ajax_nopriv_micro_office_importer_get_list_pages',array($this, 'get_list_pages_callback'));
		// Check if row will be imported in the set='part'
		add_filter('micro_office_filter_importer_import_row',			array($this, 'import_check_row'), 9, 4);
	}
	...
	function prepare_vars() {
		...
		$this->options['debug'] = micro_office_param_is_on(micro_office_get_theme_option('debug_mode'));
	    $this->options = apply_filters('micro_office_filter_importer_options', $this->options);
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			micro_office_admin_add_menu_item('theme', array(
				'page_title' => esc_html__('Install Dummy Data', 'micro-office'),
				'menu_title' => esc_html__('Install Dummy Data', 'micro-office'),
	...
	function admin_scripts() {
		wp_enqueue_style(  'micro_office-importer-style',  micro_office_get_file_url('core/core.importer/core.importer.css'), array(), null );
		wp_enqueue_script( 'micro_office-importer-script', micro_office_get_file_url('core/core.importer/core.importer.js'), array('jquery'), null, true );	
	}
	...
	function build_page() {
		...
		if ( isset($_POST['exporter_action']) ) {
			if ( !wp_verify_nonce( micro_office_get_value_gp('nonce'), admin_url() ) )
				$this->error = esc_html__('Incorrect WP-nonce data! Operation canceled!', 'micro-office');
		...
		<div class="trx_importer">
			...
			<div class="trx_importer_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Micro Office Importer', 'micro-office'); ?></h2>

				<form id="trx_importer_form">

					<?php if (count($this->options['files']) > 1) { ?>
						<p><b><?php esc_html_e('Select the demo-data set to be imported:', 'micro-office'); ?></b></p>
					...
					<div class="trx_importer_demo_set">
						<label><input type="radio"<?php if ($this->options['demo_set']=='part') echo ' checked="checked"'; ?> value="part" name="demo_set" /><?php esc_html_e('Only pages, forms and sliders', 'micro-office'); ?></label>
						<div class="trx_importer_description trx_importer_description_part<?php if ($this->options['demo_set']!='part') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>Only pages, forms layouts and sliders will be imported in this case.</b>', 'micro-office')); ?></li>
								<li><?php echo wp_kses_data(__('The imported data will be added to the existing content.', 'micro-office')); ?></li>
								<li><?php echo wp_kses_data(__('All the images are replaced with placeholders.', 'micro-office')); ?></li>
								<li><?php echo wp_kses_data(__('The new pages provide sample markup and shall not be included in the menu! You have to do it yourself.', 'micro-office')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'micro-office')); ?></li>
							</ol>
						</div>
						<label><input type="radio"<?php if ($this->options['demo_set']=='full') echo ' checked="checked"'; ?> value="full" name="demo_set" /><?php esc_html_e('Whole demo-site content', 'micro-office'); ?></label>
						<div class="trx_importer_description trx_importer_description_full<?php if ($this->options['demo_set']!='full') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>It is recommended for new installations of WordPress</b> (without posts, pages and any other data)!', 'micro-office')); ?></li>
								<li><?php echo wp_kses_data(__('As a result, you will get a new set of posts, pages, menu items - <b>a complete copy of our demo site</b>.', 'micro-office')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'micro-office')); ?></li>
							</ol>
						</div>
					</div>

					<p><b><?php esc_html_e('Select the elements to be imported:', 'micro-office'); ?></b></p>

					<?php
					$this->show_importer_params(array(
						'slug' => 'posts',
						'title' => esc_html__('Import posts, pages, taxonomies, etc.', 'micro-office'),
						'part' => 1,
						'checked' => true,
					...
					$this->show_importer_params(array(
						'slug' => 'uploads',
						'title' => esc_html__('Import media', 'micro-office'),
						'part' => 0,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					$this->show_importer_params(array(
						'slug' => 'tm',
						'title' => esc_html__('Import Theme mods', 'micro-office'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'to',
						'title' => esc_html__('Import Theme Options', 'micro-office'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'tpl',
						'title' => esc_html__('Import Templates Options', 'micro-office'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'widgets',
						'title' => esc_html__('Import Widgets', 'micro-office'),
						'part' => 1,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					do_action('micro_office_action_importer_params', $this);
					?>
	
					<div class="trx_buttons">
						<input type="button" value="<?php esc_attr_e('Start import', 'micro-office'); ?>">
					</div>
				</form>
				
				<div id="trx_importer_progress" class="notice notice-info style_<?php echo esc_attr($this->options['demo_style']); ?>">
					<h4 class="trx_importer_progress_title"><?php esc_html_e('Import demo data', 'micro-office'); ?></h4>
					<table border="0" cellpadding="4">
					<?php
					$fields = array(
						'posts'		=> esc_html__('Posts', 'micro-office'),
						'uploads'	=> esc_html__('Media', 'micro-office'),
						'tm'		=> esc_html__('Theme Mods', 'micro-office'),
						'to'		=> esc_html__('Theme Options', 'micro-office'),
						'tpl'		=> esc_html__('Template Options', 'micro-office'),
						'widgets'	=> esc_html__('Widgets', 'micro-office'),
					);
					foreach ($fields as $slug=>$title) {
						$this->show_importer_fields(array('slug' => $slug, 'title' => $title));
					}
					do_action('micro_office_action_importer_import_fields', $this);
					?>
					</table>
					<h4 class="trx_importer_progress_complete"><?php esc_html_e('Congratulations! Data import complete!', 'micro-office'); ?> <a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('View site', 'micro-office'); ?></a></h4>
				</div>
			...
			<div class="trx_exporter_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Micro Office Exporter', 'micro-office'); ?></h2>

				<?php 
				if ($this->error) {
					?><div class="trx_exporter_error notice notice-error"><?php micro_office_show_layout($this->error); ?></div><?php
				}
				?>
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					if ( isset($_POST['exporter_action']) ) { 
						...
						$fields = array(
							'users'			=> esc_html__('Users', 'micro-office'),
							'posts'			=> esc_html__('Posts', 'micro-office'),
							'theme_mods'	=> esc_html__('Theme Mods', 'micro-office'),
							'theme_options'	=> esc_html__('Theme Options', 'micro-office'),
							'templates_options'	=> esc_html__('Templates Options', 'micro-office'),
							'widgets'		=> esc_html__('Widgets', 'micro-office'),
						);
						foreach ($fields as $slug=>$title) {
							$this->show_exporter_fields(array('slug' => $slug, 'title' => $title));
						}
						do_action('micro_office_action_importer_export_fields', $this);
						?></table><?php
						...
						if (false && count($this->options['files']) > 1) {
							...
							?><p><b><?php esc_html_e('Select the demo type to be exported', 'micro-office'); ?></b></p><?php
						...
						?>
						<div class="trx_buttons">
							<input type="submit" value="<?php esc_attr_e('Export Demo Data', 'micro-office'); ?>">
						</div>
						<?php
					}
					?>
	...
	function show_importer_params($args=array()) {
		...
		<label<?php if (!empty($args['class'])) echo ' class="'.esc_attr($args['class']).'"'; ?>>
			<input type="checkbox"
					...
					echo (isset($args['checked']) && $args['checked']) || (in_array($args['slug'], micro_office_storage_get('required_plugins')) && $this->options['plugins_initial_state'])
								? ' checked="checked"' 
								: '';
					?>
					value="1"
			...
			<?php micro_office_show_layout($args['title']); ?>
		</label>
		<?php
	}
	...
	function show_importer_fields($args=array()) {
		...
		<tr class="import_<?php echo esc_attr($args['slug']); ?>">
			<td class="import_progress_item"><?php micro_office_show_layout($args['title']); ?></td>
	...
	function show_exporter_fields($args=array()) {
		...
		?>
		<tr>
			<th align="left"><?php micro_office_show_layout($args['title']); ?></th>
			<td><a download="<?php echo esc_attr($args['slug']); ?>.txt" href="<?php echo esc_url(micro_office_get_file_url('core/core.importer/export/'.trim($args['slug']).'.txt')); ?>"><?php esc_html_e('Download', 'micro-office'); ?></a></td>
		</tr>
		<?php
	}
	...
	function check_required_plugins($list='') {
		$not_installed = '';
		if (in_array('trx_utils', micro_office_storage_get('required_plugins')) && !defined('TRX_UTILS_VERSION') )
			$not_installed .= 'Micro Office Utilities';
		$not_installed = apply_filters('micro_office_filter_importer_required_plugins', $not_installed, $list);
		if ($not_installed) {
			$this->error = '<b>'.esc_html__('Attention! For correct installation of the selected demo data, you must install and activate the following plugins: ', 'micro-office').'</b><br>'.($not_installed);
			return false;
		}
		return true;
	}
	...
	function exporter() {
		...
		// Export theme mods
		micro_office_fpc(micro_office_get_file_dir('core/core.importer/export/theme_mods.txt'), serialize($this->prepare_data(get_theme_mods())));

		// Export theme, templates and categories options and VC templates
		$rows = $wpdb->get_results( $wpdb->prepare( 
												"SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s",
												 micro_office_storage_get('options_prefix') . '_options%'
												 )
									);
		$options = array();
		if (is_array($rows) && count($rows) > 0) {
			foreach ($rows as $row) {
				$options[$row->option_name] = micro_office_unserialize($row->option_value);
			}
		}
		...
		micro_office_fpc(micro_office_get_file_dir('core/core.importer/export/theme_options.txt'), serialize($this->prepare_data($options)));

		// Export templates options
		$rows = $wpdb->get_results( $wpdb->prepare(
												"SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s",
												micro_office_storage_get('options_prefix').'_options_template_%'
												)
									);
		$options = array();
		if (is_array($rows) && count($rows) > 0) {
		...
		micro_office_fpc(micro_office_get_file_dir('core/core.importer/export/templates_options.txt'), serialize($this->prepare_data($options)));
		...
		micro_office_fpc(micro_office_get_file_dir('core/core.importer/export/widgets.txt'), serialize($this->prepare_data($options)));

		// Export posts
		micro_office_fpc(micro_office_get_file_dir('core/core.importer/export/posts.txt'), serialize(array(
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				$rows[$k]['user_pass']		= '';
				$rows[$k]['display_name']	= sprintf(esc_html__('User %d', 'micro-office'), $v['ID']);
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				else if ($v['meta_key'] == 'first_name')			$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'micro-office'), $v['user_id']);
				else if ($v['meta_key'] == 'last_name')				$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'micro-office'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_first_name')	$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'micro-office'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_last_name')		$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'micro-office'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_email')			$rows[$k]['meta_value'] = 'user'.$v['user_id'].'@user-mail.net';
			}
		}
		$users['usermeta'] = $rows;
		micro_office_fpc(micro_office_get_file_dir('core/core.importer/export/users.txt'), serialize($users));

		// Export Theme specific post types
		do_action('micro_office_action_importer_export', $this);
	...
	function export_dump($table) {
		...
		if ( count( $wpdb->get_results( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->prefix . trim($table) ), ARRAY_A ) ) == 1 ) {
			...
			if ($this->options['debug']) dfl(sprintf(__("Export %d rows from table '%s'. Used query: %s", 'micro-office'), count($rows), $table, $query));
		}
		return $rows;
	}
	...
	// Import dummy data
	//-----------------------------------------------------------------------------------
	//Handler of the add_action('wp_ajax_micro_office_importer_start_import',			array($this, 'importer'));
	//Handler of the add_action('wp_ajax_nopriv_micro_office_importer_start_import',	array($this, 'importer'));
	function importer() {

		if ($this->options['debug']) dfl(__('AJAX handler for importer', 'micro-office'));

		if ( !isset($_POST['importer_action']) || !wp_verify_nonce( micro_office_get_value_gp('ajax_nonce'), admin_url('admin-ajax.php') ) )
			die();
		...
		if ($this->options['debug']) dfl( sprintf(__('Dispatch action: %s', 'micro-office'), $this->action) );
		...
		$admin_tm = max(0, min(1800, (int) micro_office_get_theme_option('admin_dummy_timeout')));
		...
		// Import posts and users
			...
			if ($this->response['result'] >= 100 && $this->options['demo_set']=='full') {
				$this->import_users();
				do_action('micro_office_action_importer_after_import_posts', $this);
			}
		...
		// Import Theme specific posts
			...
			do_action('micro_office_action_importer_import', $this, $this->action);
		}
		...
		if ($this->options['debug']) dfl( sprintf(__("AJAX handler finished - send results to client: %s", 'micro-office'), json_encode($this->response)) );
	...
	function clear_tables() {
		...
		if ($this->options['demo_set']=='full') {
			if (micro_office_strpos($_POST['demo_parts'], 'posts')!==false) {
				if ($this->options['debug']) dfl( __('Clear tables', 'micro-office') );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->posts}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "posts".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->postmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "postmeta".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->comments}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "comments".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->commentmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "commentmeta".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->terms}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "terms".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_relationships}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "term_relationships".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_taxonomy}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "term_taxonomy".', 'micro-office' ) . ' ' . ($res->get_error_message()) );
			}
			do_action('micro_office_action_importer_clear_tables', $this, $_POST['clear_tables']);
		}
	}
	...
	function import_users() {
		if ($this->options['debug']) 
			dfl(__('Import users', 'micro-office'));
		$this->response['start_from_id'] = 0;
		$this->import_dump('users', esc_html__('Users', 'micro-office'));
	}

	// Import posts, terms and comments
	function import_posts() {
		if ($this->options['debug']) 
			dfl(__('Import posts, terms and comments', 'micro-office'));
		...
		$this->import_dump('posts', esc_html__('Posts', 'micro-office'));
	}

	// Import media (uploads folder)
	function import_uploads() {
		if ($this->options['debug']) 
			dfl(__('Import media', 'micro-office'));
		...
		if ( ($txt = get_option('micro_office_import_uploads')) == '' ) {
			if ( ($txt = $this->get_file($this->options['files'][$this->options['demo_type']]['file_with_uploads'])) === false)
				return;
			else
				update_option('micro_office_import_uploads', $txt);
		}
		...
		foreach ($files as $file) {
			...
			if (micro_office_get_file_ext($file)=='zip') {
			...
			} else if (substr(micro_office_get_file_ext($file), 0, 1)==='0' && (int) micro_office_get_file_ext($file) > 0) {
				if ( ($txt = $this->get_file($file, round(max(0, $counter-1) / count($files) * 100))) === false)
					$need_exit = true;
				else {
					$zip = $this->uploads_dir.'/import_media.tmp';
					micro_office_fpc($zip, $txt, $file==$files[0] ? 0 : FILE_APPEND);
			...
			// Check time
			if ($this->options['debug']) 
				dfl(sprintf( __('File %s imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'micro-office'), $file, $result.'%', $this->max_time, time() - $this->start_time));
			// Unzip file
			if ($need_extract) {
				if (!empty($zip) && file_exists($zip)) {
					if ($this->options['debug']) 
						dfl(sprintf(__('Extract zip-file "%s"', 'micro-office'), $zip));
					if ( !unzip_file($zip, $this->uploads_dir) ) {
						$msg = sprintf(__('Error when unzip file "%s"', 'micro-office'), $zip);
					...
					$msg = sprintf(__('File "%s" not found', 'micro-office'), $zip);
					$this->response['error'] = $msg;
					if ($this->options['debug']) 
						dfl($msg);
				}
		...
		if ($result >= 100) update_option('micro_office_import_uploads', '');
	...
	function import_theme_mods() {
		if ($this->options['debug']) 
			dfl(__('Import Theme mods', 'micro-office'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'micro-office'), $this->options['files'][$this->options['demo_type']]['file_with_mods']));
		}
	}
	...
	function import_theme_options() {
		if ($this->options['debug']) 
			dfl(__('Import Theme Options', 'micro-office'));
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $k=>$v) {
				...
				update_option( $k, apply_filters('micro_office_filter_import_theme_options', $v, $k) );
			}
		} else {
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'micro-office'), $this->options['files'][$this->options['demo_type']]['file_with_options']));
		}
	}
	...
	function import_widgets() {
		if ($this->options['debug']) 
			dfl(__('Import Widgets', 'micro-office'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'micro-office'), $this->options['files'][$this->options['demo_type']]['file_with_widgets']));
		}
	}
	...
	function import_dump($slug, $title) {
		if ($this->options['debug']) 
			dfl(sprintf(__('Import dump file: "%s"', 'micro-office'), $this->options['files'][$this->options['demo_type']]['file_with_' . $slug]));
		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				if ($this->options['debug']) dfl(sprintf(__('Process table "%s"', 'micro-office'), $table));
				...
				if ($this->options['demo_set']=='part' && $table=='posts' && $this->response['start_from_id'] > 0) {
					$this->part_replace = get_option('micro_office_importer_part_replace', array());
					$this->part_image = get_option('micro_office_importer_part_image', array());
				}
				if (is_array($rows) && ($posts_all=count($rows)) > 0) {
					...
					foreach ($rows as $row) {
						...
						if (!apply_filters('micro_office_filter_importer_import_row', $this->options['demo_set']=='full', $table, $row, $this->options['demo_parts'])) continue;
						// Replace demo URL to current site URL
						$row = micro_office_replace_site_url($row, $this->options['files'][$this->options['demo_type']]['domain_demo']);
						$f = '';
						$v = '';
						...
						if ($this->options['debug']) {
							dfl( sprintf( __('Record (ID=%s) is imported. Progress: %s. Time: %s sec. from %s sec.', 'micro-office'),
											!empty($row['ID']) 
				...
				if ($this->options['demo_set']=='part' && $table=='posts') {
					update_option('micro_office_importer_part_replace', $result < 100 ? $this->part_replace : array());
					update_option('micro_office_importer_part_image', $result < 100 ? $this->part_image : array());
				}
				if ($this->options['debug']) dfl(sprintf(__('Imported %s. Elapsed time %s sec. of %s sec.', 'micro-office'), $result.'%', time() - $this->start_time, $this->max_time));
				if ($break) break;
			}
		} else {
			if ($this->options['debug']) 
				dfl(sprintf(__('Error unserialize data from the file %s', 'micro-office'), $this->options['files'][$demo_type]['file_with_' . $slug]));
		}
		$this->response['result'] = $result;
	}
	
	// Check if the row will be imported
	// Handler of the add_filter('micro_office_filter_importer_import_row', array($this, 'import_check_row'), 9, 4);
	...
	function import_prepare_no_image() {
		$no_image_title = esc_html__('No-Image placeholder', 'micro-office');
		$no_image_post = get_page_by_title($no_image_title, OBJECT, 'attachment');
		if ( empty($no_image_post->ID) ) {
			if ( ($img = micro_office_get_file_dir("images/no-image.jpg")) != '') {
				// Copy to the 'uploads' folder
				$this->part_image = wp_upload_bits( 'no-image.jpg', 0, micro_office_fgc($img));
	...
	function get_list_pages_callback() {
		if ( !wp_verify_nonce( micro_office_get_value_gp('ajax_nonce'), admin_url('admin-ajax.php') ) )
			die();

		$this->prepare_vars();

		$response = array(
			'error' => empty($_POST['demo_type']) ? esc_html__('Incorrect parameters', 'micro-office') : '',
		);
	...
	function replace_uploads($str) {
		return micro_office_replace_uploads_url($str);
	}
	...
	function prepare_data($str) {
		$need_ser = false;
		if (is_string($str) && micro_office_substr($str, 0, 2)=='a:') {
		...
		if (is_array($str) && count($str) > 0) {
			...
			if ($this->options['files'][$this->options['demo_type']]['domain_dev']!=$this->options['files'][$this->options['demo_type']]['domain_demo'])
				$str = str_replace(
							micro_office_get_domain_from_url($this->options['files'][$this->options['demo_type']]['domain_dev']),
							micro_office_get_domain_from_url($this->options['files'][$this->options['demo_type']]['domain_demo']),
							$str);
	...
	function download_file($fname, $result=0) {
		...
		if (substr($fname, 0, 5)=='http:' || substr($fname, 0, 6)=='https:') {
			...
			if (is_string($response)) {
				...
				if ($this->options['debug']) 
					dfl(sprintf(__('Download file %s successful', 'micro-office'), $fname));
			} else {
				if ($attempt < 3) {
					$this->response['attempt'] = $attempt;
					$this->response['result'] = $result;
					if ($this->options['debug']) {
						$error_log = sprintf(__("Attempt %d. Download file '%s' failed.", 'micro-office'), $attempt, $fname);
						dfl($error_log);
					}
				} else {
					unset($this->response['attempt']);
					$this->response['error'] = sprintf(__("Error download file '%s'.", 'micro-office'), $fname)
												. " \n" . __("Please, try again!", 'micro-office');
					if ($this->options['debug']) 
			...
			$rez = micro_office_get_file_dir($fname);
		}
		return $rez;
	}
	...
	function get_file($fname, $result=0) {
		$attempt = !empty($_POST['attempt']) ? (int) $_POST['attempt']+1 : 1;
		$fname = trailingslashit($this->options['demo_url']) . trim($this->options['demo_type']) . '/' . trim($fname);
		$txt = micro_office_fgc($fname, true);
		if (empty($txt)) {
			if ($attempt < 3) {
				$this->response['attempt'] = $attempt;
				$this->response['result'] = $result;
				if ($this->options['debug']) {
					$error_log = sprintf(__("Attempt %d. Load data from the file '%s' failed. ", 'micro-office'), $attempt, $fname);
					dfl($error_log);
				}
			} else {
				unset($this->response['attempt']);
				$this->response['error'] = sprintf(__("Error load data from the file '%s'.", 'micro-office'), $fname)
											. " \n" . __("Please, try again!", 'micro-office');
				if ($this->options['debug']) 
					dfl($this->response['error']);
			}
			...
			unset($this->response['attempt']);
			if ($this->options['debug']) 
				dfl(sprintf(__('Load data from the file %s successful', 'micro-office'), $fname));
		}
		return $txt;
	}
}
