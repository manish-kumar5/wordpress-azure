...
( function( $ ) {
	...
	if (!micro_office_previewer_vars['need_refresh']) {
		var $style = $('#micro_office-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="micro_office-customizer-css"></'+'style'+'>' ).find('#micro_office-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			micro_office_set_cookie(micro_office_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
