<?php
/**
 * Micro Office Framework: Registered Users
 *
...
// Theme init
if (!function_exists('micro_office_users_theme_setup')) {
	add_action( 'micro_office_action_before_init_theme', 'micro_office_users_theme_setup' );
	function micro_office_users_theme_setup() {

		// Social Login support
        add_filter( 'trx_utils_filter_social_login', 'micro_office_social_login');
		
		if ( is_admin() ) {
			// Add extra fields in the user profile
			add_action( 'show_user_profile',		'micro_office_add_fields_in_user_profile' );
			add_action( 'edit_user_profile',		'micro_office_add_fields_in_user_profile' );
	
			// Save / update additional fields from profile
			add_action( 'personal_options_update',	'micro_office_save_fields_in_user_profile' );
			add_action( 'edit_user_profile_update',	'micro_office_save_fields_in_user_profile' );
		}
...
if (!function_exists('micro_office_social_login')) {
	function micro_office_social_login($sc) {
		return micro_office_get_theme_option('social_login');
	}
}


// Return (and show) user profiles links
if (!function_exists('micro_office_show_user_socials')) {
	function micro_office_show_user_socials($args) {
		...
		if (count($list) > 0) {
			$output = '<div class="sc_socials sc_socials_size_'.esc_attr($args['size']).' sc_socials_shape_'.esc_attr($args['shape']).' sc_socials_type_' . esc_attr($args['style']) . '">' 
							. trim(micro_office_prepare_socials($list, $args['style'])) 
						. '</div>';
...
if (!function_exists('micro_office_add_fields_in_user_profile')) {
	function micro_office_add_fields_in_user_profile( $user ) { 
	?>
		<h3><?php esc_html_e('User Position', 'micro-office'); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="user_position"><?php esc_html_e('User position', 'micro-office'); ?>:</label></th>
				<td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
					<span class="description"><?php esc_html_e('Please, enter your position in the company', 'micro-office'); ?></span>
				</td>
			</tr>
		</table>
	
		<h3><?php esc_html_e('Social links', 'micro-office'); ?></h3>
		<table class="form-table">
		<?php
		...
		if (is_array($social_list) && count($social_list) > 0) {
			foreach ($social_list as $soc) {
				...
				if (!empty($sn)) {
					?>
					<tr>
						<th><label for="user_<?php echo esc_attr($sn); ?>"><?php micro_office_show_layout(micro_office_strtoproper($sn)); ?>:</label></th>
						<td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
							<span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'micro-office'), micro_office_strtoproper($sn)); ?></span>
						</td>
					</tr>
					<?php
				}
...
if (!function_exists('micro_office_save_fields_in_user_profile')) {
	function micro_office_save_fields_in_user_profile( $user_id ) {
