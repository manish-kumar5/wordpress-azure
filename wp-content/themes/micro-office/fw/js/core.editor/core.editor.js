...
jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(MICRO_OFFICE_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == MICRO_OFFICE_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(MICRO_OFFICE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				micro_office_message_success('', MICRO_OFFICE_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(MICRO_OFFICE_STORAGE['strings']['editor_caption_close']);
			} else {
				micro_office_message_warning(rez.error, MICRO_OFFICE_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		micro_office_message_confirm(MICRO_OFFICE_STORAGE['strings']['editor_delete_post'], MICRO_OFFICE_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(MICRO_OFFICE_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					micro_office_message_success('', MICRO_OFFICE_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = MICRO_OFFICE_STORAGE['site_url'];
						}, 1000);
				} else {
					micro_office_message_warning(rez.error, MICRO_OFFICE_STORAGE['strings']['editor_delete_error']);
				}
			});
