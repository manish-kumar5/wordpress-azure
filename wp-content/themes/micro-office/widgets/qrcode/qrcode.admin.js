...
jQuery(document).ready(function(){
	"use strict";
	micro_office_qrcode_init();
	micro_office_color_picker();
});

function micro_office_qrcode_init() {
	"use strict";
	jQuery('#widgets-right .widget_qrcode:not(.inited)').each(function() {
		...
		widget.find('input.iColorPicker:not(.colored)').each(function() {
			...
			if (obj.attr('id').indexOf('__i__') < 0) {
				...
				micro_office_set_color_picker(obj.attr('id'));
			}
		});
		widget.find('div.qrcode_tabs').tabs();
		widget.on('click', '.fld_button_draw', function() {
			micro_office_qrcode_update(widget);
		});
...
function micro_office_qrcode_update(widget) {
	"use strict";
	micro_office_qrcode_show(widget, {
...
function micro_office_qrcode_show(widget, vc, opt) {
	"use strict";
	if (opt.show_what==1) {
...
if (!window.micro_office_set_color_picker) {
	"use strict";
	function micro_office_set_color_picker(id_picker) {
