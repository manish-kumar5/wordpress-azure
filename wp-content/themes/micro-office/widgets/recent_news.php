...
// Theme init
if (!function_exists('micro_office_widget_recent_news_theme_setup')) {
	add_action( 'micro_office_action_before_init_theme', 'micro_office_widget_recent_news_theme_setup', 1 );
	function micro_office_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('micro_office_exists_visual_composer') && micro_office_exists_visual_composer())
			add_action('micro_office_action_shortcodes_list_vc','micro_office_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('micro_office_widget_recent_news_load')) {
	add_action( 'widgets_init', 'micro_office_widget_recent_news_load' );
	function micro_office_widget_recent_news_load() {
		register_widget('micro_office_widget_recent_news');
	}
}


// Widget Class
class micro_office_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'micro-office'));
		parent::__construct( 'micro_office_widget_recent_news', esc_html__('Micro Office - Recent News', 'micro-office'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = micro_office_sc_recent_news( array(
			'from_widget'		=> true,
		...
		if (!empty($output)) {
			...
			if ($widget_title) micro_office_show_layout($before_title . $widget_title . $after_title);
	...
	function form($instance) {
		...
		$news_styles = micro_office_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'micro-office'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'micro-office'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'micro-office'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'micro-office'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'micro-office'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show', 'micro-office'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_0"><?php esc_html_e('Hide', 'micro-office'); ?></label>
		</p>
...
if ( !function_exists( 'micro_office_sc_recent_news' ) ) {
	function micro_office_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (micro_office_param_is_on($show_categories)) {
			if ( !micro_office_storage_isset('categories_'.$category) ) {
				micro_office_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = micro_office_prepare_widgets_args(micro_office_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (micro_office_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (micro_office_param_is_on($show_categories) && !micro_office_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(micro_office_param_is_on($show_categories) && !micro_office_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = micro_office_storage_get('categories_'.$category);
			if (micro_office_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'micro-office').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'micro-office')
		...
		$args = micro_office_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('micro_office_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	micro_office_require_shortcode("trx_recent_news", "micro_office_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('micro_office_widget_recent_news_reg_shortcodes_vc')) {
	
	function micro_office_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", "micro-office"),
				"description" => wp_kses_data( __("Insert recent news list", "micro-office") ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("List style", "micro-office"),
						"description" => wp_kses_data( __("Select style to display news list", "micro-office") ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", "micro-office") ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", "micro-office"),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", "micro-office") ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", "micro-office"),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", "micro-office") ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						"param_name" => "cat",
						"heading" => esc_html__("Category", "micro-office"),
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", "micro-office") ),
						"group" => esc_html__('Query', 'micro-office'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", "micro-office"),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", "micro-office"),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", "micro-office") ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'micro-office'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", "micro-office") ),
						"group" => esc_html__('Query', 'micro-office'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'micro-office'),
						"class" => "",
						"value" => array_flip(micro_office_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'micro-office'),
						"class" => "",
						"value" => array_flip(micro_office_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", "micro-office"),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", "micro-office") ),
						"group" => esc_html__('Query', 'micro-office'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
