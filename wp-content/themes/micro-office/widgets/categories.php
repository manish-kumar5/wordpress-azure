...
// Theme init
if (!function_exists('micro_office_widget_categories_theme_setup')) {
	add_action( 'micro_office_action_before_init_theme', 'micro_office_widget_categories_theme_setup', 1 );
	function micro_office_widget_categories_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('micro_office_exists_visual_composer') && micro_office_exists_visual_composer())
			add_action('micro_office_action_shortcodes_list_vc','micro_office_widget_categories_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('micro_office_widget_categories_load')) {
	add_action( 'widgets_init', 'micro_office_widget_categories_load' );
	function micro_office_widget_categories_load() {
		register_widget( 'micro_office_widget_categories' );
	}
}

// Widget Class
class micro_office_widget_categories extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_subcategories', 'description' => esc_html__('Display subcategories list', 'micro-office') );
		parent::__construct( 'micro_office_widget_subcategories', esc_html__('Micro Office - Subcategories list', 'micro-office'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_subcategories_inner">
			<?php
			if ( $d ) {
				$cat_args['show_option_none'] = esc_html__('Select Category', 'micro-office');
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('root')); ?>"><?php esc_html_e('Root category:', 'micro-office'); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id('dropdown')); ?>"><?php esc_html_e( 'Display as dropdown', 'micro-office' ); ?></label><br />

		<input type="checkbox" class="checkbox" id="<?php echo esc_attr($this->get_field_id('count')); ?>" name="<?php echo esc_attr($this->get_field_name('count')); ?>"<?php checked( $count ); ?> />
		<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e( 'Show post counts', 'micro-office' ); ?></label><br />

		<input type="checkbox" class="checkbox" id="<?php echo esc_attr($this->get_field_id('hierarchical')); ?>" name="<?php echo esc_attr($this->get_field_name('hierarchical')); ?>"<?php checked( $hierarchical ); ?> />
		<label for="<?php echo esc_attr($this->get_field_id('hierarchical')); ?>"><?php esc_html_e( 'Show hierarchy', 'micro-office' ); ?></label>
		</p>
		<?php
	}
}
...
if ( !function_exists( 'micro_office_sc_widget_categories' ) ) {
	function micro_office_sc_widget_categories($atts, $content=null){	
		...
		extract($atts);
		$type = 'micro_office_widget_categories';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_categories' 
								. (micro_office_exists_visual_composer() ? ' vc_widget_categories wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, micro_office_prepare_widgets_args(micro_office_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_categories', 'widget_categories') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('micro_office_shortcode_output', $output, 'trx_widget_categories', $atts, $content);
	}
	micro_office_require_shortcode("trx_widget_categories", "micro_office_sc_widget_categories");
}


// Add [trx_widget_categories] in the VC shortcodes list
if (!function_exists('micro_office_widget_categories_reg_shortcodes_vc')) {
	
	function micro_office_widget_categories_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Categories", "micro-office"),
				"description" => wp_kses_data( __("Display the subcategories list for the specified category", "micro-office") ),
				...
				"params" => array(
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Show posts", "micro-office"),
						"description" => wp_kses_data( __("Show posts number in the each category", "micro-office") ),
						"class" => "",
						"std" => "1",
						"value" => array(esc_html__('Show posts number', 'micro-office') => "1"),
						"type" => "checkbox"
					),
					array(
						"param_name" => "dropdown",
						"heading" => esc_html__("Show dropdown", "micro-office"),
						"description" => wp_kses_data( __("Show categories as dropdown list", "micro-office") ),
						"class" => "",
						"std" => "0",
						"value" => array(esc_html__('Show dropdown', 'micro-office') => "1"),
						"type" => "checkbox"
					),
					array(
						"param_name" => "hierarchical",
						"heading" => esc_html__("Show hierarchical", "micro-office"),
						"description" => wp_kses_data( __("Show categories as hierarchical list", "micro-office") ),
						"class" => "",
						"std" => "1",
						"value" => array(esc_html__('Show hierarchical', 'micro-office') => "1"),
						"type" => "checkbox"
					),
					array(
					...
					array(
						...
						"description" => wp_kses_data( __("Select parent category. If empty - show all categories", "micro-office") ),
						"class" => "",
