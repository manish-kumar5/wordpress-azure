...
// Theme init
if (!function_exists('micro_office_widget_recent_reviews_theme_setup')) {
	add_action( 'micro_office_action_before_init_theme', 'micro_office_widget_recent_reviews_theme_setup', 1 );
	function micro_office_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('micro_office_exists_visual_composer') && micro_office_exists_visual_composer())
			add_action('micro_office_action_shortcodes_list_vc','micro_office_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('micro_office_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'micro_office_widget_recent_reviews_load' );
	function micro_office_widget_recent_reviews_load() {
		register_widget('micro_office_widget_recent_reviews');
	}
}

// Widget Class
class micro_office_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'micro-office'));
		parent::__construct( 'micro_office_widget_recent_reviews', esc_html__('Micro Office - Recent reviews', 'micro-office'), $widget_ops );
...
if ( !function_exists( 'micro_office_sc_widget_recent_reviews' ) ) {
	function micro_office_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'micro_office_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, micro_office_prepare_widgets_args(micro_office_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('micro_office_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	micro_office_require_shortcode("trx_widget_recent_reviews", "micro_office_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('micro_office_widget_recent_reviews_reg_shortcodes_vc')) {
	function micro_office_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", "micro-office"),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", "micro-office") ),
