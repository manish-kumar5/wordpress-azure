...
// Theme init
if (!function_exists('micro_office_widget_popular_posts_theme_setup')) {
	add_action( 'micro_office_action_before_init_theme', 'micro_office_widget_popular_posts_theme_setup', 1 );
	function micro_office_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('micro_office_exists_visual_composer') && micro_office_exists_visual_composer())
			add_action('micro_office_action_shortcodes_list_vc','micro_office_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('micro_office_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'micro_office_widget_popular_posts_load' );
	function micro_office_widget_popular_posts_load() {
		register_widget('micro_office_widget_popular_posts');
	}
}

// Widget Class
class micro_office_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'micro-office'));
		parent::__construct( 'micro_office_widget_popular_posts', esc_html__('Micro Office - Most Popular & Commented Posts', 'micro-office'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? micro_office_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = micro_office_storage_get('options_prefix').'_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = micro_office_storage_get('options_prefix').'_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'micro-office'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'micro-office'); ?></label>
...
if ( !function_exists( 'micro_office_sc_widget_popular_posts' ) ) {
	function micro_office_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'micro_office_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (micro_office_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, micro_office_prepare_widgets_args(micro_office_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('micro_office_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	micro_office_require_shortcode("trx_widget_popular_posts", "micro_office_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('micro_office_widget_popular_posts_reg_shortcodes_vc')) {
	
	function micro_office_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", "micro-office"),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", "micro-office") ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", "micro-office"),
						"description" => wp_kses_data( __("Most popular tab title", "micro-office") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", "micro-office"),
						"description" => wp_kses_data( __("Most commented tab title", "micro-office") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", "micro-office"),
						"description" => wp_kses_data( __("Most liked tab title", "micro-office") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
