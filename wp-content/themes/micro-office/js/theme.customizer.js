...
"use strict";
function micro_office_theme_customizer(option, val) {
	"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = MICRO_OFFICE_STORAGE['co_add_params']!=undefined ? MICRO_OFFICE_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#micro_office-customizer-styles-'+option).length > 0 ? jQuery('#micro_office-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="micro_office-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
