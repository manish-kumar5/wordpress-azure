...
if (!function_exists('trx_utils_support_projects_post_type')) {
	add_action( 'trx_utils_custom_post_type', 'trx_utils_support_projects_post_type', 10, 2 );
	function trx_utils_support_projects_post_type($name, $args=false) {
		if ($name=='projects') {
			if ($args===false) {
				$args = array(
					'label'               => esc_html__( 'Projects', 'trx_utils' ),
					'description'         => esc_html__( 'Projects Description', 'trx_utils' ),
					'labels'              => array(
						'name'                => esc_html__( 'Projects', 'trx_utils' ),
						'singular_name'       => esc_html__( 'Project', 'trx_utils' ),
						'menu_name'           => esc_html__( 'Projects', 'trx_utils' ),
						'parent_item_colon'   => esc_html__( 'Parent Item:', 'trx_utils' ),
						'all_items'           => esc_html__( 'All Projects', 'trx_utils' ),
						'view_item'           => esc_html__( 'View Item', 'trx_utils' ),
						'add_new_item'        => esc_html__( 'Add New Project', 'trx_utils' ),
...
if (!function_exists('trx_utils_support_projects_taxonomy')) {
	add_action( 'trx_utils_custom_taxonomy', 'trx_utils_support_projects_taxonomy', 10, 2 );
	function trx_utils_support_projects_taxonomy($name, $args=false) {
		if ($name=='projects_group') {
			if ($args===false) {
				$args = array(
					'post_type' 		=> 'projects',
					'hierarchical'      => true,
					'labels'            => array(
						'name'              => esc_html__( 'Projects Group', 'trx_utils' ),
						...
						'menu_name'         => esc_html__( 'Projects Group', 'trx_utils' ),
					),
					'show_ui'           => true,
					'show_admin_column' => true,
					'query_var'         => true,
					'rewrite'           => array( 'slug' => 'projects_group' )
					);
			}
